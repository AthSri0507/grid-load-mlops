...
def smape_masked(y_true, y_pred, min_abs=None):
    """SMAPE (%) computed only where |y_true| >= min_abs.
       min_abs defaults to the 5th percentile of |y_true| (>= 1e-3)."""
...
def wape(y_true, y_pred):
    """Weighted Absolute Percentage Error (%) = MAE normalized by sum(|y_true|)."""
...
def nrmse(y_true, y_pred):
    """Normalized RMSE (%) by the (max-min) of y_true."""
    y_true = np.asarray(y_true, dtype=float); y_pred = np.asarray(y_pred, dtype=float)
...
def eval_all(y_true, y_pred):
    return {
        ...
        "SMAPE_%": smape_masked(y_true, y_pred),  # masked, robust
...
def main():
    ...
    if "Predicted Load (kW)" in df.columns:
        df = df.drop(columns=["Predicted Load (kW)"])  # avoid leakage
    ...
    # Drop NaNs from lags/rolls and shifted target
    ...
    # Diagnostics: zeros/near-zeros
    zeros_pct = (np.abs(y_test_real.ravel()) < 1e-6).mean() * 100.0
    print(f"[diagnostic] % of near-zero targets in test set: {zeros_pct:.2f}%")
    ...
    lstm = Sequential([LSTM(128, return_sequences=True, input_shape=(n_steps, X_seq_tr.shape[-1])),
    ...
    gru = Sequential([GRU(128, return_sequences=True, input_shape=(n_steps, X_seq_tr.shape[-1])),
                      Dropout(0.2), GRU(64), Dense(1)])
